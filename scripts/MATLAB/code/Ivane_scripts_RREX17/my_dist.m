...
% DIST    Computes distance (in meters) and bearing between points on the earth
...
rowvec = 0; %don't transpose at the end
if size(lat,1)==1,
    rowvec=1; 
    lat=lat(:); %1 column
    long=long(:);
elseif size(lat,2)==2 && size(lat,1)>2, 
    rowvec=1;
    lat=lat';  %2 rows (NN=2)
    long=long';     
elseif min(size(lat))>2, error('1 or 2 vectors only in lat,long'); 
end;
if any(size(lat)~=size(long))
...
if double_row,
    dd2=[diff(cos(long).*cos(lat))' diff(sin(long).*cos(lat))' diff(sin(lat))'];
    dd2=sum(dd2.*dd2,2)';
else
    dd2=[cos(long).*cos(lat) sin(long).*cos(lat) sin(lat)];
    dd2=sum(diff(dd2).*diff(dd2),2);
end
if ( any(abs(dd2-2) < 2*((B-A)/A))^2 ), %#ok<BDLOG>
