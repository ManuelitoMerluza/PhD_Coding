function [N2_S, p_mid] = N2S(SA,CT,p,lat)
...
% MODIFICATIONS MADE BY MANUEL TORRES-GODOY, ALLOWING FOR SEPARATING THE
%  ALPHA AND BETA COMPONENTS OF N2

N2_T = (grav_local.*grav_local).*(rho_mid./(db2Pa*dp)).*(-alpha_mid.*dCT);
N2_S = (grav_local.*grav_local).*(rho_mid./(db2Pa*dp)).*(beta_mid.*dSA);
