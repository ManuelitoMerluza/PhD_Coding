...
imgname={'05Ttri_ridge.png','06.Ttri_south.png','07.Ttri_ovide.png','08.Ttri_north.png'};
...
load('RREX_T_tri_ridge.mat'); % Loads the cumulated transports


% 2015
cruise='RREX2015 ';
load([path2015 'transport_RREX15_' section '_polyfit.mat'])
T_sum=zeros(1,length(T_up_bott_tr)+1); T_sum(2:end)=T_up_bott_tr;
...
load([path2017 'transport_RREX17_' section '_polyfit.mat'])
T_sum=zeros(1,length(T_up_bott_tr)+1); T_sum(2:end)=T_up_bott_tr;
T_sum=cumsum(T_sum); % This is for centering the start of the transport at zero
X_sum=[X_ctd(1); X];
points=[49, 62];
...
title([figtitle ' Up-Triangle Transport'],'FontName','LMRoman10','FontSize',15,'FontWeight','bold')
L=legend('show');
L.AutoUpdate = 'off';
