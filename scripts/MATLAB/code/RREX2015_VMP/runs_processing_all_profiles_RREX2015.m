...
info.dp = 4; % Size of bin
info.dpD = 8; % Size of bin where dissipation is calculated
info.dpGr = 8; % Size of bin where backround gradients are calculated
info.dpTr = 4; % Size of bin of tracers
info.prof_dir = 'down'; %%CHANGE: up or down
info.fmaxT = 30;
info.k_HP_cut_T = 0.; % Makes a highpass filter for a particular frequency
info.Tmethod = 'B'; % Only option
info.Tspec = 'K'; % B or K model for the temperature spectra
info.system = 'Oce'; % Oce or lake name (does not apply)
info.Nfft = 512; % Name of the Fast Fourier Transform
...
save('RREX2015_processed_VMP6000_ManuTest.mat', '-struct' , 'OUTPUT')
