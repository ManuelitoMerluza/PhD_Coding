...
ficmlt=input('Nom du fichier Multistation  ? ','s');
...
if strfind(ficmlt,'PRES')
    if ioxy
     [lat,lon,juld,statnum,direction,pres,temp,psal,oxy] = read_nc_HYDRO_mlt(nc,'PRES','TEMP','PSAL',paramoxy);
       else
     [lat,lon,juld,statnum,direction,pres,temp,psal] = read_nc_HYDRO_mlt(nc,'PRES','TEMP','PSAL');
    end
else
    if ioxy
...
figure
plot(temp,-pres)
title([plot_title ' TEMP']) 
eval(['print -depsc2 ' [plotpath ficmlt(1:end-3) '_TEMP.eps']]);
...
pref = 0;
tpot   = tetai (pres,temp,psal,pref);
tpot = tpot';
