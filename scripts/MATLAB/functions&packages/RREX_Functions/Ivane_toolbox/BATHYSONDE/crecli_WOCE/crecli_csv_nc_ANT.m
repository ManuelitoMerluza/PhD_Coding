...
%
% ----------------------------------------------------------
% programme utilisé pour les campagnes ANT_V2 et ANT_XI2
% format des ficheirs csv différent des autres

% param : TPOT,PRES,TEMP,PSAL : on ne traite pas TPOT
% pas de flag
...
% pas de flag
% on ne reecrit pas theta (1er param)

clear tvalcli tvalbon

if strcmp(sect_hyd,'ANT_XI2')
    N=5;
else
    N=4;
end
...
while ~ifin
         ...
         if strcmp(cell2mat(c{1}(1)),'END_DATA')
             ...
             nlevels = nlevels + 1;
             tvalcli(1,nlevels) =   str2double(cell2mat(c{1}(2)));
             tvalcli(2,nlevels) =   str2double(cell2mat(c{1}(3)));
             tvalcli(3,nlevels) =   str2double(cell2mat(c{1}(4)));
         end
end
...
isok1 = find(tvalcli(2,:) ~=-9.999);
isok2 = find(tvalcli(3,:) ~=-9.999);
...
ncid=netcdf.create([sect_hyd 'd' cstat '_cli.nc'],'NC_CLOBBER');
...
netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'),'PROBE_TYPE','CTD Neil Brown, MARK III B');
if strcmp(sect_hyd,'ANT_XI2')
    netcdf.putAtt(ncid,netcdf.getConstant('NC_GLOBAL'),'PROBE_NUMBER','SN1123');
else
...
% Ecriture des flags de qualite.
    ...
    iflag1(1:nparam)=1; 
    clear flag_nc
    flag_nc(1:nparam,1:nlevels)=2;
    
for jj=1:nparam
    % flag CCHDO absent : flag cli = 2 et not calibrated
    
    i9 = find(tvalbon(jj,:) == -999 | tvalbon(jj,:) == -9.999);
    if ~isempty(i9)
        i99=i9
    end
