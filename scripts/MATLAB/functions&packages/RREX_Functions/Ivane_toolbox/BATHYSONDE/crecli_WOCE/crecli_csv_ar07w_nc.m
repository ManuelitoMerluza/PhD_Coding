...
% (.csv en _cli.nc)
%
% Catherine Lagadec / decembre 2014 après le CDD de Mathieu Hamon
%
% ----------------------------------------------------------
% programme utilisé pour les campagnes 
% AR07W_2003 et AR07W_2005
...
while ~ifin
         ...
         if strcmp(cell2mat(c{1}(1)),'END_DATA')
             ...
             nlevels = nlevels + 1;
                 ...
                 tvalcli(5,nlevels) =   str2double(cell2mat(c{1}(9)));
                 tvalcli(6,nlevels) =   str2double(cell2mat(c{1}(11)));
                 tvalcli(7,nlevels) =   str2double(cell2mat(c{1}(13)));
                 ...
                 tflag(5,nlevels) =  str2double(cell2mat(c{1}(10)));
                 tflag(6,nlevels) =  str2double(cell2mat(c{1}(12)));
                 tflag(7,nlevels) =  str2double(cell2mat(c{1}(14)));
         end
end
...
inan = find(tvalcli(2,:) == -999. | tflag(2,:) == 0);
tvalcli(2,inan) = NaN;
inan = find(tvalcli(3,:) == -999. | tflag(3,:) == 0);
tvalcli(3,inan) = NaN;
inan = find(tvalcli(4,:) == -999. | tflag(4,:) == 0);
tvalcli(4,inan) = NaN;
inan = find(tvalcli(5,:) == -999. | tflag(5,:) == 0);
tvalcli(5,inan) = NaN;
inan = find(tvalcli(6,:) == -999. | tflag(6,:) == 0);
tvalcli(6,inan) = NaN;
inan = find(tvalcli(7,:) == -999. | tflag(7,:) == 0);
tvalcli(7,inan) = NaN;

isok1=find(isfinite(tvalcli(2,:)));
isok2=find(isfinite(tvalcli(3,:)));
isok=intersect(isok1,isok2);
...
%calcul RMS sur PSAL
%iok3=find(isfinite(tvalcli(3,:)));
iok3=find(tvalcli(3,:) > -999.);
rms_psalcalc(i) = rms(tvalcli(3,iok3) -tvalcli(6,iok3));

% calcul RMS sur TEMP
%iok2=find(isfinite(tvalcli(2,:)));
iok2=find(tvalcli(2,:) > -999.);
rms_tempcalc(i) = rms(tvalcli(2,iok2) -tvalcli(5,iok2));

% calcul RMS sur OXYK
%iok4=find(isfinite(tvalcli(4,:)) | );
iok4=find(tvalcli(4,:) > -999.);
rms_oxykcalc(i) = rms(tvalcli(4,iok4) -tvalcli(7,iok4));
if rms_oxykcalc(i) > 4
a=rms_oxykcalc(i)
end
...
comment = ['EXPOCODE= ' expocode  '   sect_hyd = ' sect_hyd  '    line = ' line1];
...
for jj=1:nparam
    ...
    % dans AR07W (2003 et 2005), les valeurs à -999 sont flaguées à 0 !!!
    % ça change !!
...
save fic rms_psalcalc rms_tempcalc rms_oxykcalc
