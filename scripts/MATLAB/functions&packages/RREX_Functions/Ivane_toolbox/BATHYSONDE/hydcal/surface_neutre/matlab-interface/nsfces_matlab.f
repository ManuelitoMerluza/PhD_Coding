...
ccc
	...
	call system('/usr/bin/rm -f ns_multiples.dat')
...
ccc
	...
	open(10,file='ns-m-lvls.in',status='old')
	...
	if(ng.gt.ngmax)
     &		stop 'ERROR 1 in nsfces-matlab.f: ngmax parameter too small'
...
ccc
	...
	open(10,file='ns-m-stpg.in',status='old')
	open(11,file='nsfces_matlab1.out',status='unknown')
	open(12,file='nsfces_matlab2.out',status='unknown')
	...
	do while (.TRUE.)
	  ...
	  n = nint(xn)
	  if(n.gt.nz)
     &		stop 'ERROR 2 in nsfces-matlab.f: nz parameter too small'
	  do k = 1,n
...
cc
	  ...
	  if(ierr.eq.1) then
	    ...
	    if(ierr_wr.eq.0) then
	      open(13,file='ns_multiples.dat',status='new')
	      ierr_wr = 1
	    else
	      open(13,file='ns_multiples.dat',status='old',access='append')
	    end if

	    open(14,file='ns-multiples.dat',status='old')
...
10	close(10)
	...
	if(ierr_wr.eq.1) call system('/usr/bin/rm -f ns-multiples.dat')
