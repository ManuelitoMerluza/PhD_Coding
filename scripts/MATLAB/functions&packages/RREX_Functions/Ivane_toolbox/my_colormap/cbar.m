function [nc,ax,tleg]=cbar(ax,ext,units,clim,nc,fs,eq,txtplace);
...
% applique.
%
% [nc,ax,tleg]=cbar(ax,ext,units,clim,nc,fs)
%   donne en sortie le nb de cases effectif nc, 
%   le handle des axes ax et le handle des textes tleg
...
%      tleg(i)=text(4,i,num2str(xtextval(i)),'FontSize',fs,'Horizo','right');
      tleg(i)=text(2.5,ix,num2str(xtextval(i)),'FontSize',fs,'FontWeight','b','Horizo','left','Vertical','middle');
   else
