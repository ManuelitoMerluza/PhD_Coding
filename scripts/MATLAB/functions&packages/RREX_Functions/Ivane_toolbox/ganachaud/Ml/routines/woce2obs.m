...
if p_woce
  ...
  else %WOCE FORMAT /JADE FORMAT
    if exist('jadefile')
      disp('Reading JADE format')
      [oslat,oslon,ostnnbr,Time,obotd,botdat,varname,varunits]=...
	w2o_readjadebot(jadefile);
    elseif exist('civafile')
      disp('Reading CIVA format')
      [oslat,oslon,ostnnbr,Time,obotd]=w2o_readcivahdr(civafile);
    else
...
if p_alison
  ...
  opres=opres(:,gisel);
  for iprop=1:nvar
    eval([opropnm{iprop} '=' opropnm{iprop} '(:,gisel);'])
  end
  castno=ones(size(opres));
  sampno=ones(size(opres));
  ...
  figure(2);clf
  plot(sw_dist(oslat,oslon,'km'),'+-');grid on;zoom;xlabel('stat number');
  ylabel('distance between stations (km)');title(Cruise)
  zoom on; set(gca, 'xlim', [0 length(oslon)])
  land;setlargefig
  
  figure(3);clf
  plot(-obotp,'+-');grid on;zoom;xlabel('stat number');
  ylabel('bottom pressure');title(Cruise)
  zoom on;set(gca, 'xlim', [0 length(oslon)])
  land;setlargefig
  s10=input('print these figures ? ','s');
  if s10=='y'
    f1;land;setlargefig;pg ;
    f2;land;setlargefig;pg;
    f3;land;setlargefig;pg;
  end
  
  if exist('orstomdir')|p_alison
...
for isec=1:nsec
  ...
  setlargefig
  s10=input('print ? ','s');if s10=='y',pg,end
...
%CAN RUN SPOTOUTLIER AGAIN TO DOUBLE CHECK AFTER CHANGES
w2o_spotoutliers
