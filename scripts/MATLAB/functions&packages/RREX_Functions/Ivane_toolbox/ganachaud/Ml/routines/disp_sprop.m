...
% INPUT DIRECTORY:
  ...
  if ~exist('Idir') 	Idir = '/data17/diana/Alex/Hydrosys/EN129/Data/'; 	end
  
% INPUT DATA HEADER FILE:
if ~exist('Ihdr') 	Ihdr = 'EN129_SM_hdr'; 		end
...
% for iprop = ipvec
 for iprop = 1:Nprop
% INPUT PROPERTY FILE:  
  prop = rhydro([Idir Propfiles(iprop,:)], Precision(iprop,:), ...
...
if (do_plot == 1)
  plot_sprop(prop, Propnm(iprop,:), Propunits(iprop,:), Nstat, ...
      Cruise, dst, Pres, Maxd(:,iprop), Botd, Vcont);
