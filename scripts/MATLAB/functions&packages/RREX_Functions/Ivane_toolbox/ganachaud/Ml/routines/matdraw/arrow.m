...
%        will convert line objects to corresponding arrows by taking the first
%        and last points in each line object as the Start and Stop points of a
%        new arrow.  Note that simply ARROW(H) will update the arrows if the
%        current view has changed.  Note also that the object handles vector
%        can be specified as a property as ARROW('ObjectHandles',H,...).
%
...
% Copyright (c)1995, Erik A. Johnson <johnsone@uxh.cso.uiuc.edu>, 6/14/95

        % NOTE: if using Matlab <4.2c, FINDOBJ is buggy (crashes Matlab if
        %       >100 objects are found) or nonexistent, so the line
        %               objs = findobj;
        %       should be replaced by
        %               objs=0; k=1;
        %               while (k<=length(objs)),
        %                 objs = [objs; get(objs(k),'Children')];
        %                 if (strcmp(get(objs(k),'Type'),'axes')),
        %                   objs=[objs;get(objs(k),'XLabel');get(objs(k),'YLabel');...
        %                              get(objs(k),'ZLabel');get(objs(k),'Title')];
        %                 end;
        %                 k=k+1;
        %               end;
        % NOTE: if using Matlab <4.2, where the 'Tag' property does not exist,
        %       delete all instances of
        %               ,'Tag',ArrowTag
        %       and replace all instances of
        %               strcmp(get(oh,'Tag'),ArrowTag)
        %       with
        %               all(size(ud)==[1 15])

% Many thanks to Keith Rogers <kerog@sp.isl.secom.co.jp> for his many
% excellent suggestions and beta testing.  Check out his shareware
% package MATDRAW.  He has permission to distribute ARROW with MATDRAW.
...
if (nargin==1),
        if (isstr(arg1)),
                ...
                if (strcmp(arg1(1:4),'prop')),
                        c = sprintf('\n');
                        disp(['ARROW Properties:' c c ...
                              ' Default values are given in [square brackets], and other acceptable' c ...
                              ' equivalent property names are in (parenthesis).' c c ...
                              ...
                              '      P = | .  .  .  |    P1 = |  .    .    .   |    P2 = | .  .  .  |' c ...
                              '          | .  .  .  |         |  .    .    .   |         | .  .  .  |' c ...
                              '          \ xN yN zN /         \ xN-1 yN-1 zN-1 /         \ xN yN zN /']);
...
if (isstr(start     )),  s=lower([start(:)'      '   ']); if (all(s(1:3)=='def')), start      = defstart;      end; end;
if (isstr(stop      )),  s=lower([stop(:)'       '   ']); if (all(s(1:3)=='def')), stop       = defstop;       end; end;
if (isstr(len       )),  s=lower([len(:)'        '   ']); if (all(s(1:3)=='def')), len        = deflen;        end; end;
if (isstr(baseangle )),  s=lower([baseangle(:)'  '   ']); if (all(s(1:3)=='def')), baseangle  = defbaseangle;  end; end;
if (isstr(tipangle  )),  s=lower([tipangle(:)'   '   ']); if (all(s(1:3)=='def')), tipangle   = deftipangle;   end; end;
if (isstr(wid       )),  s=lower([wid(:)'        '   ']); if (all(s(1:3)=='def')), wid        = defwid;        end; end;
if (isstr(crossdir  )),  s=lower([crossdir(:)'   '   ']); if (all(s(1:3)=='def')), crossdir   = defcrossdir;   end; end;
if (isstr(page      )),  s=lower([page(:)'       '   ']); if (all(s(1:3)=='def')), page       = defpage;       end; end;
if (isstr(ends      )),  s=lower([ends(:)'       '   ']); if (all(s(1:3)=='def')), ends       = defends;       end; end;
if (isstr(linewidth )),  s=lower([linewidth(:)'  '   ']); if (all(s(1:3)=='def')), linewidth  = deflinewidth;  end; end;
...
if (isstr(oldh      )),  s=lower([oldh(:)'       '   ']); if (all(s(1:3)=='def')), oldh       = [];            end; end;
...
if (all(size(ii))),
        if (strcmp(computer,'MAC2')), c=13; else, c=10; end;
        s = args(ii',:);
...
if (all(size(ii))),
        ...
        if (all(isreal(crossdir(ii)))),
                crossdir(ii) = real(log10(crossdir(ii)));
        else,
                jj = find(isreal(crossdir(ii)));
