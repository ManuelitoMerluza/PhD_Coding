...
% Zoom UserData
%
% UserData(1)     X axis zoom active toggle
% UserData(2)     Y axis zoom active toggle
% UserData(3)     Z axis zoom active toggle
% UserData(4)     Magnification Level
% UserData(5)     Direction (0 = out, 1 = in)
% UserData(6)     Interruptible flag
...
if(isstr(command))
	...
	elseif(strcmp(command,'Setup Pick'))
		if(strcmp(get(gcf,'interruptible'),'yes'))
			UserData(6) = 1;
		else
			UserData(6) = 0;
		end
		...
		if(length(axis) == 4)
			...
			delete(findobj('Tag','MatDraw Zoom'));
			storage = store(pfig,4,'MatDraw Zoom');
	...
	elseif(strcmp(command,'3D ButtonUp'))	
		storage = sort(findobj('Tag','MatDraw Zoom'));
		Data = get(storage(4),'UserData');
		mdzoom(Data(4:6));
		delete(storage);
				...
				if(length(currax) == 4 | isempty(ObjList))
					...
					for(i=1:size(ObjList,1))
						...
						elseif(strcmp(get(ObjList(i,2),'Type'),'surface'))
							meshsize = size(Data,2);
							Xdata = Data(1:meshsize,:);
							Xdata = Xdata(find(~isnan(Xdata)));
							Ydata = Data(meshsize+1:2*meshsize,:);
							Ydata = Ydata(find(~isnan(Ydata)));
							Zdata = Data(2*meshsize+1:3*meshsize,:);
							Zdata = Zdata(find(~isnan(Zdata)));
						end
						minx = min([minx min(min(Xdata))]);
						maxx = max([maxx max(max(Xdata))]);
