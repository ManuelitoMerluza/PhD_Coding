...
if(command == 0)
	fig = gcf;
	men = gcm;
	if(strcmp(get(fig,'MenuBar'),'figure'))
		submenu = get(men,'UserData');
...
elseif(command == 1)
	...
	if(strcmp(computer,'MAC2')) % Work around bug in Mac Matlab v4.2c
		...
		uimenu(workspace,'Label','Quit', 'Callback','exit','Separator','on');
	else
...
elseif(command == 2)
	...
	set(findobj(fig,'Label','Quit'),'Accelerator','Q');
