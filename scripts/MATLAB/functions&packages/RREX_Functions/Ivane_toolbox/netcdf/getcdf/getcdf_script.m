function values = getcdf_script(file, varid, corner, end_point, ...
...
%
% DESCRIPTION:
% getcdf_script is an non-interactive function that gets a hyperslab
% of values from a netCDF variable; its arguments specify what data
% points are to be retrieved.  Thus, getcdf_script is suitable for use
...
%    to the case for the old getcdf_batch.  This change was made so that
...
% to getcdf_script - sensible defaults are available for the rest.
% These are:
...
% 
% EXAMPLE:
%  us = getcdf_script('uvd', 's-u', [1 1 1], [720 1 151], [2 1 3], 2, 3, 1000);
...
%     Date     $Date: 1996/11/19 06:29:48 $
%     RCSfile  $RCSfile: getcdf_script.m,v $
% @(#)getcdf_script.m   1.8   92/08/05
% 
%--------------------------------------------------------------------
...
if varid < 0 | varid >= nvars
   error([ 'getcdf_script was passed varid = ' int2str(varid) ])
end

if all ( order ~= [ 1 2 ] )
   error([ 'getcdf_script was passed order = ' int2str(order) ])
end

if all ( change_miss ~= [ 1 2 3 ] )
   error([ 'getcdf_script was passed change_miss = ' int2str(change_miss) ])
end
...
for i = 1:nvdims
  ...
  if ( corner(i) < 0 |  end_point(i) < 0 )
    ...
    if corner(i) >= sizem | end_point(i) < 0 | end_point(i) >= sizem
      s = [ 'getcdf_script was passed corner = ' int2str(corner(i)+1) ...
...
if change_miss ~= 1
  ...
  if len_index_miss > 0
	...
	values = setstr(values);
      ...
      s = [ 'getcdf_script was passed change_miss = ' int2str(change_miss) ];
      error(s)
    end
  end
end
