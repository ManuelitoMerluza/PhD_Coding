...
% getcdf_script which reads the netcdf file in a non-interactive way.
...
if nargin == 0
  ...
  return
elseif nargin == 2
  values = getcdf_script(file, varid);
  return
elseif nargin == 3
  values = getcdf_script(file, varid, corner);
  return
elseif nargin == 4
  values = getcdf_script(file, varid, corner, end_point);
  return
elseif nargin == 5
  values = getcdf_script(file, varid, corner, end_point, stride);  
  return
elseif nargin == 6
  values = getcdf_script(file, varid, corner, end_point, ...
      stride, order);
...
% store information about its initial and final values in the string s.
    ...
    rhid = check_string(name, varstring, nvars) - 1;
...
% next, get the finishing and stride point if these are required.
         ...
         if notdone
            ...
            while end_point < corner(i) | end_point > ledim(i)
               ...
               ret_val = return_val(s, end_point+1);
               end_point = ret_val - 1;
            end
