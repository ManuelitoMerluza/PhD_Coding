function [BIN,SLOW] = resolve_VMP_profile_shear_T1(DATA, inp, info,dataf_name,PLOT)
    DATA
    ...
    LT_0 = calculates_thorpe_scale(Ps,sgt,'density',info.prof_dir,0);
    BIN.LT_Gb = sqrt(pres_av(Ps,LT_0,pres,info.dpD,0));
    ...
    legend('Fit','Variance','fontsize',7,'location','southeast')
    xlabel('\chi (K^2 s^{-1})')
    set(gca,'yticklabel',[])
    set(gca,'YDir','reverse')
    set(gca,'xscale','log')
    ...
    legend('T01 (O&C)','T01 (O)','sh01 (O)','sh02 (O)','fontsize',7,'location','southeast')
    xlabel('K (m^2 s^{-1})')
    ylabel('p (db)')
    set(gca,'YDir','reverse')
    set(gca,'xscale','log')
...
%     ylim([pmin,pmax])
     ...
     hold on
     plot(BIN.LT_Gb,BIN.pres,'-','linewidth',1,'markersize',4)
     plot(BIN.LO1,BIN.pres,'-','linewidth',1,'markersize',4)
     %plot(BIN.LO2,BIN.pres,'-','linewidth',1,'markersize',4)
     legend('L_T','L_T^{Galb}','LO_{T1}','fontsize',7,'location','southeast')
     xlabel('L_T, L_O (m)')
     set(gca,'YDir','reverse')
     set(gca,'xscale','log')
     grid('on')
