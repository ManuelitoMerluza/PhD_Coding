function [BIN,SLOW,FAST] = resolve_VMP250_profile(DATA, inp, info,dataf_name,PLOT)
    ...
    if ~ isfield(info,'noisep_T2')
        info.noisep_T2 = [-10.24,-0.89,info.fAA];
    else
...
%     
    ...
    [S_JAC,Cmatch] = salinity_JAC(Ps,T_JAC,C_JAC);
    Theta_JAC = sw_ptmp(S_JAC, T_JAC, Ps,0);
    sgt = sw_pden(S_JAC, T_JAC,Ps,0);
    ...
    BIN.theta=pres_av(Ps,Theta_JAC,pres,info.dpTr,2.7);
    BIN.C = pres_av(Ps,C_JAC,pres,info.dp,2.7);
    BIN.S = pres_av(Ps,S_JAC,pres,info.dp,2.7);
    BIN.sigmat = pres_av(Ps,sgt,pres,info.dp,2.7);
    BIN.grT = mean_grad(Ps,Theta_JAC,pres,info.dpD);
    BIN.grS = mean_grad(Ps,S_JAC,pres,info.dpGr);
    sort_rho = sort(sgt);
    ...
    for i = 1:length(pres)
        ...
        %FP07 calculations
        try
            [BIN.Xiv1(i),BIN.Xic1(i),BIN.Xif1(i),BIN.KB1(i),BIN.fit_flag_T1(i),BIN.sXif1(i), BIN.sKB1(i), BIN.MAD1(i), BIN.MADf1(i),BIN.MADc1(i),BIN.LKH1(i), BIN.LKHratio1(i), BIN.maxK1(i)] =Xi_spec(Pf(jp),T1f_hp(jp),info.minKT,info.fmaxT,0.5*(BIN.KBSH1(i)+BIN.KBSH2(i)),mean(abs(Wf(jp))),info.noisep_T1,  Nfft, overlap,info.Tspec,info.Tmethod,info.time_res, info.time_res_speed, info.pole,info.hfactor,info.peak_rem_T1,PLOT);
            BIN.epsT1(i) = visco*D^2*(2*pi()*BIN.KB1(i))^4;
            BIN.epsT1max(i) = visco*D^2*(2*pi()*BIN.maxK1(i)*info.kmax_factor)^4;
        

           [BIN.Xiv2(i),BIN.Xic2(i),BIN.Xif2(i),BIN.KB2(i),BIN.fit_flag_T2(i),BIN.sXif2(i), BIN.sKB2(i), BIN.MAD2(i), BIN.MADf2(i),BIN.MADc2(i),BIN.LKH2(i), BIN.LKHratio2(i), BIN.maxK2(i)] =Xi_spec(Pf(jp),T2f_hp(jp),info.minKT,info.fmaxT,0.5*(BIN.KBSH1(i)+BIN.KBSH2(i)),mean(abs(Wf(jp))),info.noisep_T2,  Nfft, overlap,info.Tspec,info.Tmethod,info.time_res, info.time_res_speed, info.pole,info.hfactor,info.peak_rem_T2,PLOT);
    ...
    BIN.KOsbT1 = 0.2*BIN.epsT1.*(BIN.N2).^-1;
    BIN.KTf1 = 0.5*BIN.Xif1.*(BIN.grT).^-2;
    BIN.KOsbT2 = 0.2*BIN.epsT2.*(BIN.N2).^-1;
    BIN.KTf2 = 0.5*BIN.Xif2.*(BIN.grT).^-2;
    [BIN.Reb1,~,BIN.KBB1] = Bouffard_model(BIN.epsSH1,BIN.N2,BIN.T,7);
    [BIN.Reb2,~,BIN.KBB2] = Bouffard_model(BIN.epsSH2,BIN.N2,BIN.T,7);
    ...
    plot((BIN.epsT1max),BIN.pres,'-','linewidth',1,'markersize',4,'color',[0.5,0.5,0.5])
    ...
    legend('T01_f','T01_c','T02_f','T02_c','fontsize',7,'location','southeast')
    xlabel('\chi (K^2 s^{-1})')
    set(gca,'yticklabel',[])
    set(gca,'YDir','reverse')
    set(gca,'xscale','log')
