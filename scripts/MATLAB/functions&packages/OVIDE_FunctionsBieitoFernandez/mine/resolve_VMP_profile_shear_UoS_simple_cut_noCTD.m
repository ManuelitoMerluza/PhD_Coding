function [BIN,SLOW] = resolve_VMP_profile_shear_UoS_simple_cut_noCTD(DATA, info,dataf_name,PLOT)
    DATA
...
%     end
    ...
    %T_SB_up = DATA.SBT1(iips_u);
    %C_SB_up = DATA.SBC1(iips_u);
   ...
   %calculates displacements for thorpe length
    if strcmp(info.thorpe_method,'simple')
        if ismember(info.prof_dir,'down')
            ...
            [sort_uT1, ist1] = sort(T1fth,'descend');
            displuT1 = Pfth - Pfth(ist1);

            [sort_uT2, ist2] = sort(T2fth,'descend');
            displuT2 = Pfth - Pfth(ist2);
        else


            [sort_uT1, ist1] = sort(T1fth,'ascend');
            displuT1 = Pfth - Pfth(ist1);
        ...
        BIN.LTuT1 = sqrt(pres_av(Pfth,displuT1.^2,pres,info.dpD,0));
        BIN.LTuT2 = sqrt(pres_av(Pfth,displuT2.^2,pres,info.dpD,0));
    else
        %Uses the Galbraith method
        %LT_0 = calculates_thorpe_scale(Ps,sgt,'density',info.prof_dir,PLOT);
        LTuT1_0 = calculates_thorpe_scale(Pfth,T1fth,'temperature',info.prof_dir,PLOT);
        LTuT2_0 = calculates_thorpe_scale(Pfth,T2fth,'temperature',info.prof_dir,PLOT);
        %BIN.LT = sqrt(pres_av(Ps,LT_0,pres,info.dpD,0));
        BIN.LTuT1 = sqrt(pres_av(Pfth,LTuT1_0,pres,info.dpD,0));
        BIN.LTuT2 = sqrt(pres_av(Pfth,LTuT2_0,pres,info.dpD,0));
    end
...
%     ylim([pmin,pmax])
     ...
     legend('L_T(1)','L_T(2)','L_O_1','L_O_2','fontsize',7,'location','southeast')
     xlabel('L_T, L_O (m)')
     set(gca,'YDir','reverse')
     set(gca,'xscale','log')
     grid('on')
